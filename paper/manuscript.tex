...
for development, an 892-protein human set for the main results, and a set of 5,984
proteins curated from 12 organisms spanning eukaryota, bacteria, and archaea, of which
the 5,555 with complete structure and annotation were analysed, for the universality
test. Structures are AlphaFold-DB models of a single release
...
a maximum of 0.42 at layer 24, then falls to a minimum of 0.156 at layer 35,
...
layer 35 (Figure~\ref{fig:fig1}a). These metrics are converged at the sample size
used (Figure S1) and sit far above label-permutation nulls at every layer
(Figure S2).
...
(Figure~\ref{fig:fig2}a; the full condition-by-condition matrix at seven depths is
shown in Figure S5). Sequence, the one channel carrying information that cannot
...
0.01 to 0.07 across all 48 layers, while structure reaches a CKA of about 0.99 with
...
an effective rank of 2.8 to 218 across layers. One alternative explanation is
...
InterPro residue-site annotations available for 631 of the 892 proteins. Per-residue
...
and the four function-versus-physical pairs (bottom rows) stay near a CKA of 0.01
throughout, while the function-to-all pair is warm only at the input, where the all
condition still carries the function signal.
...
below a CKA of 0.55 with the physical modalities.}
\label{fig:fig2}
\end{figure}
...
index rises with depth from 0.18 to 0.59, mirroring the pooled curve
...
(0.18 to 0.59), mirroring the pooled curve. (c) Effective rank of the whole cloud
...
Across 5,555 proteins from 12 organisms spanning all three superkingdoms (Figure S6),
the pooled fusion curve is nearly identical to the human-only result, with a silhouette maximum
...
with a standard deviation of 0 (Figure S4). Superkingdoms are intermixed in the
representation rather than separated, so the organisation is by modality and not by
source organism (Figure S7). Multimodal fusion is therefore a
...
All analysis code is available at \url{https://github.com/JLSteenwyk/esm3-modality-atlas}. The
...
deposited at figshare (\url{https://doi.org/FIGSHARE.DOI}).
